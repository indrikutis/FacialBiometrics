...
def create_merged_df(tool_results, utkface_dataset, tool, merged_df_name):
    ...
    if (tool).lower() == 'deepface':
        ...
        merged_df = pd.merge(tool_results, utkface_dataset, how='inner', left_on='Image name', right_on='image_name')
        merged_df = merged_df[merged_df['Notes_x'].isna() & merged_df['Notes_y'].isna()]

    else:
        merged_df = pd.merge(tool_results, utkface_dataset, how='inner', left_on='image_name', right_on='image_name')
...
def calculate_accuracy(merged_df, age_range, tool):
    ...
    if (tool).lower() == 'deepface':
        ...
        # predicted_age_lower = merged_df['age_group'].apply(lambda x: int(str(x).split('-')[0]) if isinstance(x, str) and '-' in x else int(str(x).split('.')[0]))
        # predicted_age_upper = merged_df['age_group'].apply(lambda x: int(x.split('-')[1]) if isinstance(x, str) and '-' in x else int(str(x).split('.')[0]))
...
deepface_results = pd.read_excel("Results/FairFace_analysis_results_UTKFace_0.3.xlsx")
...
merged_df = create_merged_df(deepface_results, utkface_dataset, tool, merged_df_name)
